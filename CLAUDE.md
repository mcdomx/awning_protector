...
`WeatherClient` tracks `_last_obs_at` on every `obs_st` event. `AutomationEngine._evaluate()` reads `seconds_since_last_obs` each cycle. If > `WEATHER_TIMEOUT_S` (300s), it does not retract immediately: on the first cycle past the threshold it calls `POST {WEATHER_URL}/admin/restart` (`AutomationEngine._restart_weather_service()`) — the `tempest_weather` service's own endpoint for restarting its systemd unit — and logs `weather_timeout_restart`, then logs `weather_timeout_waiting` each cycle for up to `WEATHER_RESTART_GRACE_S` (180s) giving the service a chance to recover. Only if data is still stale once that grace period elapses does it issue undeploy and log `weather_timeout`. Normal evaluation resumes automatically (and both the restart and grace-period state reset) once data flows again — restart is retried on each new timeout episode, not repeatedly during one. A failed restart call (service unreachable, etc.) is caught and logged; the grace-period wait proceeds regardless.
